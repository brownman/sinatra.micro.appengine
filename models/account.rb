...
class Account
  ...
  def self.authenticate(username_or_email, pass)
    current_account = first(:username => username_or_email) || first(:email => username_or_email)
    return nil if current_account.nil? || Account.encrypt(pass, current_account.salt) != current_account.hashed_password
    current_account
  end
